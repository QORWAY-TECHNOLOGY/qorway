# LICENSE

© 2026 QORWAY Technology. All rights reserved.

This repository and its contents are proprietary.

The materials contained in this repository, including but not limited to concepts, architecture, documentation, system designs, naming, diagrams, frameworks, specifications, and strategic models, are the intellectual property of QORWAY Technology.

## Restrictions

You may not, without prior written permission from QORWAY Technology:

- copy, reproduce, or redistribute this material;
- modify, adapt, or create derivative works from this material;
- use this material for commercial purposes;
- implement, reverse-engineer, or replicate the described systems;
- publish, sublicense, sell, or transfer any part of this material;
- use QORWAY names, trademarks, components, or architecture in another product, company, or project.

## Proprietary Components

The following components are proprietary and protected:

- QORWAY Decision Intelligence Infrastructure
- Atlas Causal Intelligence Engine
- Domain Pack Architecture
- PolicyCore Constraint Validation Layer
- PulseFlow Protocol
- GreenCore Execution Optimization Layer
- Knowledge Graph Framework
- Decision Intelligence System
- QORWAY system diagrams, models, and specifications

## Confidentiality

Some elements of the QORWAY architecture are intentionally excluded from this repository.

Internal logic, implementation details, prompts, causal models, scoring systems, Domain Pack internals, and execution rules remain confidential and are not licensed for public use.

## No License Granted

Access to this repository does not grant any license, express or implied, to use QORWAY Technology intellectual property.

Any use requires a separate written commercial agreement.

## Contact

For licensing, partnership, or authorized use inquiries:

**QORWAY Technology**  
Sovereign Decision Intelligence Infrastructure for Europe

```
---
*© QORWAY Technology SAS — www.qorway.com*  
*Proprietary. All rights reserved.*
```
